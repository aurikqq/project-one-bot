...
from functions.send_to_chat import *
...
dp.message.register(get_bot_logs, Command("get_logs"))
dp.message.register(send_message_to_chat, Command("send_msg"))
...
async def test(message: Message):
    result = await profile_collection.update_many(
        {'game': {}, "game.played": "", "game.times_played": 0, "game.rice": 0},
        {'$set': {'game': {'played': "", 'times_played': 0, 'rice': 0}}}
    )
    await message.reply(f"{result.modified_count} полей изменены.")
...
async def on_startup():
    ...
    send_start_message = input('Send a message about enabling to chat?\n(Y - yes, N - no)\n')
    if send_start_message.lower() == 'y':
        await bot.send_message(chat_id, 'Бот запущен.')
        print('\nMessage was sent to the chat.')
    else:
        print("\nMessage wasn't sent to the chat.")
